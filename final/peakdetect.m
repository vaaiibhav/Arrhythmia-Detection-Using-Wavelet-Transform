...
% R_Rinterval = length(R_R)/(time_scale/60); % calculate heart rate


heart_rate=length(R_i)/(time_scale/60); % calculate heart rate
% msgbox(strcat('Heart-rate is = ',mat2str(heart_rate)));
heart_beat_rate = uint8(heart_rate);
if(heart_beat_rate <60)
%     msgbox('low heart beat')
end
if(heart_beat_rate >60 && heart_beat_rate <120)
%     msgbox('Normal heart beat')
end
if(heart_beat_rate > 120)
%     msgbox('High heart beat')
end
...
disp('R_Rinterval  is');
disp(((max_R_interval/fs)-(min_R_interval/fs)*60));
...
ax(1) = plot(211);plot(time(1:length(buffer_plot)),buffer_plot(1:end));   
else
ax(1) = plot(211);plot(time,buffer_plot(1:(view*fs)));
end
